...
BOOL DNSIsResolved(IP_ADDR *HostIP)
{
   ...
   switch(smDNS)
   {
      case DNS_HOME:
         ...
         //printf(usb_cdc_putc,"\n\rDNS Server: %U.%U.%U.%U ...",AppConfig.PrimaryDNSServer.v[0],AppConfig.PrimaryDNSServer.v[1],AppConfig.PrimaryDNSServer.v[2],AppConfig.PrimaryDNSServer.v[3]);
         StartTime = TickGet();
         smDNS++;
         break;
      ...
      case DNS_QUERY:
         //printf(usb_cdc_putc,"\n\rDNS_QUERY");
       #if STACK_USE_PPP      //todo shouldn't we do this for MAC also?
      ...
      case DNS_GET_RESULT:
         ...
         while(DNSHeader.Answers.Val)
         {
            ...
            //DNSGetString(NULL);  //Response name
