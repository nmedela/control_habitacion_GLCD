...
#define debug_udp(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z)
...
void UDPFlush(void)
{
    ...
    debug_udp(debug_putc, "MAC-%X:%X:%X:%X:%X:%X",
...
BOOL UDPProcess(NODE_INFO *remoteNode, IP_ADDR *localIP, WORD len)
{
   ...
   if(h.Checksum)
   {
      ...
      // In case if the end of the RX buffer is reached and a wraparound is needed, set the next address to prevent writing to the wrong address.
       ...
       {
           debug_udp(debug_putc, "INVALID-CS ");
           MACDiscardRx();
           return TRUE;
       }
   }
    ...
    if ( s == INVALID_UDP_SOCKET )
    {
         ...
         */
        debug_udp(debug_putc, "INVALID-SOCKET ");
        MACDiscardRx();
    }
    else {
    ...
    debug_udp(debug_putc, "MATCH AS:%U ", s);
